...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['network_zone'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'network_zone'=>$limit]);
        }
...
$field=$_GET['field'];
     ...
     $qry=DB::table('network_zone')->where('is_deleted',0)->where(function($query){
         $query->Orwhere('network_zone_description','like','%'.@$_GET['search'].'%');
    
 
     }) ->orderBy($field,$orderby)->paginate($limit); 
}
 else{
$qry=DB::table('network_zone') ->where('is_deleted',0)->orderBy('id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('network_zone/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Network Zone">
...
<div class="col-sm-3  1"  style="margin-top: 2px;" >
    <span data-toggle="modal" data-bs-target="#filterModal" data-target="#filterModal"> 
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-network-zone')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Network Zone">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('network_zone')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
         ...
         <img src="{{asset('/public')}}/img/network.png" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                                        ...
                                        <div class="  " style="width:55%">
                                                      ...
                                                      <div class="d-flex " style="padding-top: 20px">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >N</p>
                                                             
   <div style="box-shadow:none;padding-bottom: 0px;padding-top:2px;min-height:29px;border-radius:5px;background:{{$q->tag_back_color}};color:{{$q->tag_text_color}};min-width: 100%;max-width: 100%" class=" text-center px-2 border-none  font-size-md badgeAuto  bubble-white-new bubble-text-sec" >{{$q->network_zone_description}}</div>
                                                                          </div>
        ...
        <form class="mb-0 pb-0"  action="{{url('end-network_zone')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Revoke Network Zone</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Network Zone</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
function showData(id){
    ...
    $('.c4').css({'backgroundColor':'#D9D9D9','color':'#7F7F7F','borderColor':'#7F7F7F'})
        ...
        $.ajax({
            type:'get',
            data:{id:id},
            url:'{{url('get-network-zone-content')}}',
                dataType:'json',
    beforeSend() {
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Network Zone");
                    if(c){
                        window.location.href="{{url('delete-network-zone')}}?id="+id;
                    }
                            })  
           })
</script>
