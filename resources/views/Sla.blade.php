...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['sla'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'sla'=>$limit]);
        }
...
$field=$_GET['field'];
     ...
     $qry=DB::table('sla')->where('is_deleted',0)->where(function($query){
         $query->Orwhere('sla_description','like','%'.@$_GET['search'].'%');
    
 
     }) ->orderBy($field,$orderby)->paginate($limit); 
}
 else{
$qry=DB::table('sla') ->where('is_deleted',0)->orderBy('id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('sla/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Sla">
...
<div class="col-sm-3  1"  style="margin-top: 2px;" >
    <span data-toggle="modal" data-bs-target="#filterModal" data-target="#filterModal"> 
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-sla')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Sla">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('sla')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
         ...
         <img src="{{asset('/public')}}/img/header-black-sla.png" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                                        ...
                                        <div class="  " style="width:55%">
                                                      ...
                                                      <div class="d-flex " style="padding-top: 20px">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >S</p>
                                                             
   <div style="box-shadow:none;padding-bottom: 0px;padding-top:2px;min-height:29px;border-radius:5px;background:{{$q->tag_back_color}};color:{{$q->tag_text_color}};min-width: 100%;max-width: 100%" class=" text-center px-2 border-none  font-size-md badgeAuto  bubble-white-new bubble-text-sec">{{$q->sla_description}}</div>
                                                                          </div>
                                       ...
                                       <?php     if(Auth::check()){    
                                            if(@Auth::user()->role!='read'){ ?>
                                                                     <div class="ActionIcon px-0 ml-2    " style="border-radius: 5px"  >
                                                                         <a    href="{{url('edit-sla')}}?id={{$q->id}}" class=" "   >
        ...
        <form class="mb-0 pb-0"  action="{{url('end-sla')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Revoke Sla</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Sla</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Sla");
                    if(c){
                        window.location.href="{{url('delete-sla')}}?id="+id;
                    }
                            })  
           })
</script>
