...
$html='';
     ...
     $q=DB::table('network as a')->select('a.*','s.site_name','c.firstname','c.logo' ,'s.address','s.city','s.country','s.phone','s.zip_code','s.province' ,'usr.firstname as created_firstname','usr.lastname as created_lastname','upd.firstname as updated_firstname','upd.lastname as updated_lastname','c.logo' )->join('clients as c','c.id','=','a.client_id')->join('sites as s','s.id','=','a.site_id') ->leftjoin('users as usr','usr.id','=','a.created_by')->leftjoin('users as upd','upd.id','=','a.updated_by')->where('a.id',$id)->first();
