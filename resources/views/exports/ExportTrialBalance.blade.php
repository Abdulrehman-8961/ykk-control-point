 ...
 <table class="table border-0 table-period">
     <thead>
         <tr>
             ...
             <td class="text-right" style="font-weight:600;padding:0;border:0 !important;text-align:right;" colspan="2">
                 Trial Balance</td>
         </tr>
         <tr>
             <td style="font-weight:600;padding:0;border:0 !important;" colspan="2">
                 {{ date('d-M', strtotime(getFiscalYearEnd($client->fiscal_start))) }}-{{ $fyear }}
             </td>

             <td class="text-right" style="font-weight:600;padding:0;border:0 !important;text-align:right;"></td>

             <td class="text-right" style="font-weight:600;padding:0;border:0 !important;text-align:right;"></td>

         </tr>
         <tr>
             <td style="font-weight:600;padding:0;border:0 !important;text-align:left;" colspan="2">
                 {{ $fyear }}</td>
         ...
         <tr>
             ...
             <td class="text-right" style="font-weight:600;padding:0;border:0 !important;text-align:right;">Debits</td>
             <td class="text-right" style="font-weight:600;padding:0;border:0 !important;text-align:right;">Credits</td>
         </tr>
     </thead>
     <tbody>
         <?php
         ...
         @foreach ($reports as $j)
             <?php
             $total_debits = DB::table('journals')->where('account_no', $j->account_no)->where('fyear', $j->fyear)->where('client', $j->client)->where('is_deleted', 0)->sum('debit');
             $total_credits = DB::table('journals')->where('account_no', $j->account_no)->where('fyear', $j->fyear)->where('client', $j->client)->where('is_deleted', 0)->sum('credit');
             $debit = 0.0;
             $credit = 0.0;
