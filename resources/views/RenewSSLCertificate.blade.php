...
$qry=DB::table('ssl_certificate')->where('id',$_GET['id'])->first();
     $renew_date=date('Y-m-d');
                $expiry_date=$qry->cert_edate;
          
         if($qry->cert_rdate!=''){
                    
                               $start_date = strtotime($qry->cert_rdate);
  $end_date = strtotime(date("Y-m-d"));
$days= ($end_date - $start_date)/60/60/24;
        if($days<=90){
                  $renew_date=$qry->cert_rdate;   
                  if($qry->cert_rdate>$qry->cert_edate){
                        $expiry_date=date('Y-m-d',strtotime($qry->cert_edate.' + 365 days'));
                  }                       
        }       
    }
...
$abs_diff = $later->diff($earlier)->format("%a"); //3
  ...
  ?>
                               ...
                               <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                ...
                                <div class="ml-4">
                                <h4  class="mb-0 header-new-text" style="line-height:25px">Renewing Certificate  
</h4>
                                    ...
                                    <!--        <a href="javascript:;"  class="saveContract text-white"  data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Renew and Continue"  class="text-white"><i class="fa fa-plus texti-white"  ></i> </a> -->
                                              ...
                                              <a href="javascript:;" class="text-white saveContract" data="0" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Renew SSL Certificate"><i class="fa fa-check text-white"   ></i> </a>
...
<div class="contract_type_button">
  ...
  <label class="btn btn-new ml-5" for="a50"  value="public" >Public</label>
    ...
    <div class="block new-block PublicDiv {{$qry->cert_type=='public'?'':'d-none'}}" >
                     ...
                     name="cert_rdate" placeholder="Y-M-d" data-alt-input="true"   value="{{$renew_date}}"   data-date-format="Y-m-d" data-alt-format="Y-M-d">
                                            </div>
                                        </div>
                                        <div class="row form-group ">
   ...
   <div class="block new-block" >
                            ...
                            <div class="block-content new-block-content">
                                      ...
                                      <div class="row form-group ">
                                            ...
                                            <div class="col-sm-4">
                                                         <input type="text" class="j kr form-control bg-white" id="cert_edate" 
...
$('#cert_edate').focus()
var coned=$('#cert_edate').flatpickr()
coned.open();
...
$(document).on('click','.btnDeleteContract',function(){
    ...
    Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> SAN Deleted. <a href="javascript:;" class="  btn-notify btnContractUndo ml-4" data1='+id+' data='+key+'>Undo</a>', delay: 5000}); 
showcontractDetails();
...
$.ajax({
    ...
    success:function(res) {
        ...
        Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Certificate successfully renewed ', delay: 5000});
