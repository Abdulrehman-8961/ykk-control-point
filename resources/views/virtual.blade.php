...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
                  if($page_type==''){
                DB::table('settings')->where('user_id',Auth::id())->update(['virtual_asset'=>$limit]);
        }
        elseif($page_type=='servers'){
            DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_server'=>$limit]);
        }
        elseif($page_type=='other'){
             DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_others'=>$limit]);
        }
        elseif($page_type=='managed'){
             DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_managed'=>$limit]);
        }
        elseif($page_type=='support-contracts'){
             DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_support'=>$limit]);
        }
            elseif($page_type=='ssl-certificate'){
             DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_ssl'=>$limit]);
        }
            elseif($page_type=='inactive'){
             DB::table('settings')->where('user_id',Auth::id())->update(['asset_virtual_inactive'=>$limit]);
        }
              ...
              if($page_type==''){
                     ...
                     DB::table('settings')->insert(['user_id'=>Auth::id(),'virtual_asset'=>$limit]);
        }
        elseif($page_type=='servers'){

                      DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_server'=>$limit]);
        }
        elseif($page_type=='other'){
                       DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_others'=>$limit]);
        }
        elseif($page_type=='managed'){
                         DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_managed'=>$limit]);
        }
        elseif($page_type=='support-contracts'){
                          DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_support'=>$limit]);
        }
            elseif($page_type=='ssl-certificate'){
               DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_ssl'=>$limit]);
        }
            elseif($page_type=='inactive'){
                     DB::table('settings')->insert(['user_id'=>Auth::id(),'asset_virtual_inactive'=>$limit]);
        }
        ...
        if($no_check!=''){
            ...
            if($no_check->virtual_asset!=''){
     
            if($page_type==''){
                 $limit=$no_check->virtual_asset;
        }

        elseif($page_type=='servers'){
             if($no_check->asset_virtual_server!=''){
                 $limit=$no_check->asset_virtual_server;
        }
        elseif($page_type=='other'){
             if($no_check->asset_virtual_others!=''){
                   $limit=$no_check->asset_virtual_others;
        }
    }
        elseif($page_type=='managed'){
             if($no_check->asset_virtual_managed!=''){
                    $limit=$no_check->asset_virtual_managed;
        }
    }
        elseif($page_type=='support-contracts'){
             if($no_check->asset_virtual_support!=''){
               $limit=$no_check->asset_virtual_support;
        }
    }
            elseif($page_type=='ssl-certificate'){
                 if($no_check->asset_virtual_ssl!=''){
                   $limit=$no_check->asset_virtual_ssl;
        }
    }
            elseif($page_type=='inactive'){
                 if($no_check->asset_virtual_inactive!=''){
               $limit=$no_check->asset_virtual_inactive;
        }
    }
}
...
$qry=DB::table('assets as a')->where(function($query) use($sear){ 
 ...
 $query->OrwhereRaw("exists (select 1
     ...
     })->select('a.*','s.site_name','p.hostname as parent_asset_name','d.domain_name','c.firstname','o.operating_system_name','m.vendor_name','at.asset_icon','at.asset_type_description','n.vlan_id  as vlanId','c.logo')->join('clients as c','c.id','=','a.client_id')->join('sites as s','s.id','=','a.site_id')->leftjoin('operating_systems as o','o.id','=','a.os')->leftjoin('asset_type as at','at.asset_type_id','=','a.asset_type_id')->leftjoin('domains as d','d.id','=','a.domain')->leftjoin('vendors as m','m.id','=','a.manufacturer')->leftjoin('network as n','a.vlan_id','=','n.id')->leftjoin('assets as p','p.id','=','a.parent_asset') ->whereRaw("a.is_deleted=0 $cond")->where('a.is_deleted',0)->where('a.asset_type','virtual') ->orderBy($field,$orderby) ->paginate($limit); 
...
?> 
        <div class="col-sm-3">
                     <form class="push mb-0"   method="get" id="form-search" action="{{url('virtual/')}}?{{$filter}}">
...
<div class="col-sm-3">
    <span data-toggle="modal" data-bs-target="#filterModal" data-target="#filterModal"> 
      <button type="button" class="btn btn-dual d1 {{isset($_GET['advance_search'])?'active':''}} "   data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" title="" data-original-title="Filters"   >
                           ...
                           <button class="btn btn-dual d2    "    data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" title="" data-original-title="Export"   href="javascript:;" style="margin-left: 5px;margin-right:  0px" >
                           <img src="{{asset('public/img/ui-icon-export.png')}}" width="20px" height="20px">
                        </button>
                    </span>
                              @if(Auth::user()->role!='read') 
                         <a class="btn btn-dual d2    "    href="{{url('add-assets')}}/virtual" data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" title="" data-original-title="Add Virtual Asset">
...
<div class="   col-lg-3   " >
          {{$qry->appends($_GET)->onEachSide(0)->links()}}
                       </div><div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0" action="{{url('virtual')}}/{{$page_type}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
                    ...
                    @if($q->AssetStatus=='1')
                                           ...
                                           <div class=" bg-new-green ml-auto  badge-new  text-center   font-weight-bold  text-white"  >
                                                                                 <img src="{{asset('public')}}/img/status-white-active.png?cache=1" class="mr-2" width="15px"><span class=" ">Active</span>
                                                                    </div>  
                                ...
                                @else 
                                       <div class=" bg-new-red ml-auto  badge-new  text-center  font-weight-bold text-white"  >
                                                                                 <img src="{{asset('public')}}/img/action-white-end-revoke.png?cache=1" class="mr-2" width="15px"><span class=" ">Inactive</span>
                                                                    </div>
                                ...
                                @endif
                                   ...
                                   @if(strpos($q->operating_system_name,'Windows')!==false)
                                                                        <div class="ActionIcon OsIcons" data-src="{{asset('public')}}/img/icon-os-windows-color.png?cache=1" data-original-src="{{asset('public')}}/img/icon-os-windows-grey.png?cache=1">
                                                                           <a href="javascript:;" class="toggle" data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" data-title="{{$q->operating_system_name}}" data-html="true"   data-original-title="<span class='HostActive text-white' >{{$q->operating_system_name}}</span><br><span class=' HostActive text-yellow' >{{$q->use_}}</span>" >
                                                                        <img  src="{{asset('public')}}/img/icon-os-windows-grey.png?cache=1" data="windows"   width="20px">
                                                                    </a>
                                                                </div>
                                                                        @elseif(strpos($q->operating_system_name,'ESXi')!==false)
                                                                                     <div class="ActionIcon OsIcons" data-src="{{asset('public')}}/img/icon-os-esxi-color.png?cache=1" data-original-src="{{asset('public')}}/img/icon-os-esxi-grey.png?cache=1">
                                                                           <a href="javfiascript:;" class="toggle" data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" data-title="{{$q->operating_system_name}}" data-html="true"   data-original-title="<span class='HostActive text-white' >{{$q->operating_system_name}}</span><br><span class=' HostActive text-yellow' >{{$q->use_}}</span>" >
                                                                           <img  src="{{asset('public')}}/img/icon-os-esxi-grey.png?cache=1" data="esxi"  width="24px" >
                                                                       </a>
                                                                   </div>
                                                                        @elseif(strpos($q->operating_system_name,'Linux')!==false)
                                                                                     <div class="ActionIcon OsIcons" data-src="{{asset('public')}}/img/icon-os-linux-color.png?cache=1" data-original-src="{{asset('public')}}/img/icon-os-linux-grey.png?cache=1">
                                                                           <a href="javascript:;" class="toggle" data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" data-placement="top" data-title="{{$q->operating_system_name}}" data-html="true"   data-original-title="<span class='HostActive text-white' >{{$q->operating_system_name}}</span><br><span class=' HostActive text-yellow' >{{$q->use_}}</span>" >
                                                                           <img  src="{{asset('public')}}/img/icon-os-linux-grey.png?cache=1" data="linux"    width="26px" >
                                                                       </a>
                                                                   </div>
                                                                        @endif
                                                                         <?php $line_items=DB::Table('contract_assets as ca')->select('a.contract_no','a.contract_status','a.contract_start_date','a.contract_end_date')->where('ca.hostname',$q->id)->join('contracts  as a','a.id','=','ca.contract_id')->groupBy('a.contract_no')->where('a.is_deleted',0)->orderBy('a.contract_no','asc')->get();
...
if($abs_diff<=30){
 ...
 elseif($l->cert_status=='Inactive'){
     ...
     $cvm1.='<span class="HostInactive  ">N/A</span><br>'; 
                                                                                            ...
                                                                                            ?>
                                                                    <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-ssl-certificate-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-ssl-certificate-grey.png?cache=1">
                                                                   <a  data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" class="b  " data-placement="top" data-title="Edit" data-html="true" data-original-title="{{$cvm1}}" href="javascript:;" c>
                                                                       
                                                                        <img src="{{asset('public')}}/img/icon-ssl-certificate-grey.png?cache=1"  width="24px">
                                                                        </a>
                                                                    </div>
                                                                     <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-contract-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-contract-grey.png?cache=1">
                                                                              <a  data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" class="b  " data-placement="top" data-title="Edit" data-html="true" data-original-title="{{$cvm}}" href="javascript:;" c>
                                                                       
                                                                        <img src="{{asset('public')}}/img/icon-contract-grey.png?cache=1"  width="23px">
                                                                        </a>
                                                                        </div>

                                                                        @if($q->asset_type_description=='Physical Server')
                                                                         <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-p-asset-d-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-p-asset-d-grey.png?cache=1">
                                                                            @else
                                                                              <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-vm-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-vm-grey.png?cache=1">
                                                                            @endif

                                                                         <a  data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" class="b  " data-placement="top" data-title="Edit" data-html="true" data-original-title="<span class='HostActive text-white'>{{$q->asset_type_description}}</span><br><span  class='HostActive ' >{{$q->ip_address}}</span>" href="javascript:;" c>
                                                                       
                                                                         
                                                                        @if($q->asset_type_description=='Physical Server')
                                                                                <img src="{{asset('public')}}/img/icon-p-asset-d-grey.png?cache=1"  width="24px" >
                                                                                @else
                                                                                   <img src="{{asset('public')}}/img/icon-vm-grey.png?cache=1"  width="24px">
                                                                        @endif

                                                                        </a>
                                                                    </div>
                                                                     <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-edit-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-edit-grey.png?cache=1">
                                                                         <a  data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" class="b  " data-placement="top" data-title="Edit"  data-html="true"  data-original-title="<span class='font-10pt'>Edit</span>"  href="{{url('edit-assets')}}?id={{$q->id}}" c>
                                                                       
                                                                        <img src="{{asset('public')}}/img/icon-edit-grey.png?cache=1"   width="24px">
                                                                        </a>
                                                                    </div>
                                                                     <div class="ActionIcon" data-src="{{asset('public')}}/img/icon-trash-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-trash-grey.png?cache=1">
                                                           
                                                                           <a  data-toggle="tooltip" data-trigger="hover"  data-trigger="hover" class="btnDelete " data="{{$q->id}}" data-placement="top"  data-html="true"  data-title="Delete" data-original-title="<span class='font-10pt'>Delete</span>" href="javascript:;" c>
                                                                        <img src="{{asset('public')}}/img/icon-trash-grey.png?cache=1"  width="22px" >
                                                                        </a>
                                                                    </div>
                                                                    </div>
                                        </div>
...
<div class="modal fade" id="filterModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Filter Virtual Assets</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
 ...
 <a href="{{url('virtual')}}" class="btn     btn-new-secondary float-right" style="background: black;
    color: goldenrod;">Clear Filters</a>
                                            @else
                                                
     <a href="{{url('virtual')}}" class="btn     btn-new-secondary float-right" style="">Clear Filters</a>
                                       @endif
                        </div>
                    </div>
...
<div class="modal fade" id="EndModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
   ...
   <div class="row">
    <div class="col-sm-12">
       ...
       <form class="mb-0 pb-0" id="exportform" action="{{url('export-excel-virtual')}}?{{$filter}}" method="get" >
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export Virtual Asset</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
    ...
    <div class="col-sm-12">
        <label>Fields to Export</label>
          <select class="form-control selectpicker" data-style="btn-outline-light border columns text-dark"  id="columns" data-actions-box="true"  data-live-search="true"    data- multiple="" required="" name="columns[]" >
              ...
              <option value="8">Role/Description</option>
              <option value="9">O/S</option>
              <option value="10">Environment</option> 
              <option value="11"> D/R Plan  </option>
              <option value="12">Clustered  </option>
              <option value="13">Internet Facing </option>
              <option value="14">Load Balanced  </option>
              <option value="15">VLAN ID</option>
              <option value="16">Primary IP</option>
              <option value="17">Line: Additional IP Label</option> 
              <option value="18">Line: Additional IP Value</option>
              <option value="19">vCPU </option>
              <option value="20">Memory </option>
              <option value="21">SSL Certificate </option>
              <option value="22">Support </option>
              <option value="23">Managed </option>
              <option value="24">App Owner</option>
              <option value="25">SLA</option>
              <option value="26">Patched</option>
             <option value="27">Monitored</option>
                <option value="28">Backup</option>
                <option value="29">Anti-Virus</option>
              <option value="30">Replicated</option>
                   <option value="31">Vulnerability Scan</option>
                <option value="32">SIEM</option>
         <option value="33">SMTP</option>
          </select>
      </div>
   </div>
...
$.ajax({
    ...
    url:"{{url('change-virtual-asset-columns')}}",
    success:function(res){
        console.log(res)
    }
    ,error:function(e) {
    ...
    run(id)
       getVendor(id); 
               ...
               $(document).on('click','.btnDelete',function(){
                    ...
                    if(c){
                        window.location.href="{{url('delete-virtual-assets')}}?id="+id;
                    }
                            })  
           })
</script>
