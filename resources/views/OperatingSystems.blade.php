...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['operating_system'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'operating_system'=>$limit]);
        }
...
$qry=DB::table('operating_systems') ->where('is_deleted',0)->orderBy('id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('operating_systems/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Operating System">
...
<div class="col-sm-3  1"  style="margin-top: 2px;" >
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-operating-systems')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Operating System">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('operating-systems')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
                         ...
                         <div class="block-content pt-1 pb-1 d-flex  pl-1 position-relative">
                                            ...
                                            @if($q->operating_system_image!='')
                                                    <img src="{{asset('/public')}}/operating_system_logos/{{$q->operating_system_image}}" class="rounded-circle"  width="100%" height="60px" style=" object-fit: cover;">
                                                     @else
         <img src="{{asset('/public')}}/img/image-default.png" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                                        ...
                                        <div class="  " style="width:55%">
                                                      ...
                                                      <div class="d-flex " style="padding-top: 20px; ">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >O</p>
                                                             
                                                                         <p class="font-12pt mb-0 text-truncate   c4  "   style="max-width:100%;min-width: 100%"   data="{{$q->id}}">{{$q->operating_system_name}} </p>
                                                                       </div>
                                       ...
                                       <?php     if(Auth::check()){    
                                            if(@Auth::user()->role!='read'){ ?>
                                                                     <div class="ActionIcon px-0 ml-2    " style="border-radius: 5px"  >
                                                                         <a    href="{{url('edit-operating-systems')}}?id={{$q->id}}" class=" "   >
        ...
        <form class="mb-0 pb-0"  action="{{url('end-operating_systems')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Revoke Operating System</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Operating System</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
$('#EndModal').modal('show')
               ...
               $('#showdata').on('click','.btnEdit',function(){
                ...
                $.ajax({
                    type:'get',
                    data:{id:id},
                    url:'{{url('show-operating_systems')}}',
                    success:function(res){
                        $('#viewData').modal('show');
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Operating System");
                    if(c){
                        window.location.href="{{url('delete-operating-systems')}}?id="+id;
                    }
                            })  
           })
</script>
