...
@section('content')
                                    ...
                                    <div class="block-content py-2 pl-3 d-flex position-relative pr-2" style="">
                                        ...
                                        <div class="w-100 ">
                                            <div class="d-flex justify-content-between">
                                                <span class="font-signika bubble-item-title text-uppercase"
                        ...
                        <div class="modal-content">
                            ...
                            <div class="block  block-transparent mb-0">
                                <div class="block-content pt-0 row form-group mt-2">
                                    <label class="col-sm-4 d-flex align-items-center modal-label mandatory"
                                        for="color_img">Item
                                        Category</label>
                                    <div class="col-sm-7">
                ...
                <div class="modal fade" id="edit-item-categories-modal" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header modal-header-new align-items-center mb-2 py-2 px-4">
                                <h1 class="modal-header-insert mb-0">
                                    <span id="edit_item_category_heading" class="text-uppercase">EDIT</span><br>
                                    <span class="modal-subheader">ITEM CATEGORY</span>
                                </h1>
                                <button type="button" class="close close-cross" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                            ...
                            <div class="block block-transparent mb-0">
                                ...
                                <div class="block-content pt-0 row form-group mt-2">
                                    <label class="col-sm-4 d-flex align-items-center modal-label"
                                        for="color_img">Itemcode</label>
                                    <div class="col-sm-7">
                                        <select class="modal-input shadow-non e form-control " id="editItemSelect">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // AB
            $('#item_category').on('keydown', function(e) {
                ...
                $('#editItemSelect').select2({
                    ...
                    templateSelection: function(data, container) {
                        ...
                        if (data.id) {
                            ...
                            optionEl.attr('data-description', data.description || '');
                        }
            ...
            // Modified fetchWorkorders with query tracking
            let latestQuery = null;
            let activeRequest = null;

            function fetchWorkorders(query = '', limit = 10) {
            ...
            let debounceTimer;
            $(document).on('select2:open', '#itemSelect', function() {
                ...
                $search.off('input.select2-search').on('input.select2-search', function() {
                    ...
                    debounceTimer = setTimeout(() => {
                        ...
                        fetchWorkorders(term, term ? 0 : 10);
                    }, 300);
                });
            });
        ...
        `
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (response.itemcodes && response.itemcodes.length > 0) {
                                ...
                                <div class="col-sm-10 mx-auto mb-2 selected-item pl-0" data-id="${item.id}" data-code="${item.item_code}">
                    ...
                    if (select.val()) { // Only proceed if an item is selected
                        ...
                        if (code && id) {
                            ...
                            if (!exists) {
                                ...
                                $('#editSelectedItems').append(`
                    <div class="col-sm-10 mx-auto mb-2 selected-item pl-0" data-id="${id}" data-code="${code}">
                        <div class="selected-items-list px-3 py-2 d-flex justify-content-between align-items-center">
            ...
            $('#editAddItem').on('click', function() {
                const itemCategory = $('#edit_item_category').val().trim();
                const selectedOption = $('#editItemSelect').find(':selected');
                const code = selectedOption.val();
                const desc = selectedOption.data('description') || '';
                const id = selectedOption.data('id');
                ...
                // `);
                    ...
                    $.ajax({
                        url: "{{ url('/check-itemcode-category') }}", // <-- route to validate
                        type: "POST",
                        data: {
                        ...
                        success: function(response) {
                            if (response.exists) {
                                var message =
                                    `<img src="${warningIcon}" width="24px" class="mt-n1"> Itemcode already assigned to another category (${response.category_name}).`;
                                showCustomWarningNotification(message, "500px");
                            } else {
                                // Check if already added locally
                                const exists = $('#editSelectedItems').find(`[data-id='${id}']`)
                                    .length > 0;
        ...
        `
            ...
            $('#item_category').focusout(function() {
                var item_category = $(this).val().trim();
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.exists) {
                                ...
                                var message =
                                    `<img src="${warningIcon}" width="24px" class="mt-n1"> Item Category already exists!`;
                                showCustomWarningNotification(message, "500px");
                            } else {
            ...
            $('#item_category').on('input', function() {
                $('#addItem').prop('disabled', true);
            });
            ...
            $('#insert-item-categories-modal').on('hidden.bs.modal', function() {
                // Reset the whole form
                $('#form-insert-item-categories')[0].reset();

                // Reset Select2 dropdowns if used
                $('#itemSelect').val(null).trigger('change');

                // Clear readonly description
                $('#itemDescription').val('');
                $('#item_category').prop('readonly', false);

                // Clear selected items container
                $('#selectedItems').empty();
