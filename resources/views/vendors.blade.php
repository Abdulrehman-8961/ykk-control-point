...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['vendors'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'vendors'=>$limit]);
        }
...
$qry=DB::table('vendors') ->where('is_deleted',0)->orderBy('id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('vendors/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Vendors">
...
<div class="col-sm-3  1"  >
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-vendors')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Vendors">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('vendors')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
                         ...
                         <div class="block-content pt-1 pb-1 d-flex  pl-1 position-relative">
                                            ...
                                            @if($q->vendor_image!='')
                                                    <img src="{{asset('/public')}}/vendor_logos/{{$q->vendor_image}}" class="rounded-circle"  width="100%" height="60px" style=" object-fit: cover;">
                                                     @else
         <img src="{{asset('/public')}}/img/image-default.png" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                                        ...
                                        <div class="  " style="width:55%">
                                                      ...
                                                      <div class="d-flex " style="padding-top: 20px">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >V</p>
                                                             
                                                                         <p class="font-12pt mb-0 text-truncate    c4"  style="max-width:100%;min-width: 100%" data="{{$q->id}}">{{$q->vendor_name}}</p>
                                                                       </div>
                                       ...
                                       <?php     if(Auth::check()){    
                                            if(@Auth::user()->role!='read'){ ?>
                                                                     <div class="ActionIcon px-0 ml-2    " style="border-radius: 5px"  >
                                                                         <a    href="{{url('edit-vendors')}}?id={{$q->id}}" class=" "   >
                                                                      ...
                                                                      <div class="ActionIcon px-0 ml-2    " style="border-radius: 5px"  >
                                                                       ...
                                                                       <img src="{{asset('public')}}/img/d3-delete.png?cache=1" class="mt-n1" width="23px"  >
                                                                        </a>
        ...
        <form class="mb-0 pb-0"  action="{{url('end-vendors')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Revoke Vendors</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Vendors</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Vendors");
                    if(c){
                        window.location.href="{{url('delete-vendors')}}?id="+id;
                    }
                            })  
           })
</script>
