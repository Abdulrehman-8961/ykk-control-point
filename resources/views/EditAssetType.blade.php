...
?>
<?php 
$qry=DB::Table('asset_type')->where('asset_type_id',$_GET['id'])->first();
?>
  ...
  background-position: center;
                ...
                <!-- Hero -->
                            ...
                            <div class="block card-round   bg-new-blue new-nav" >
                               ...
                               <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                ...
                                <div class="ml-4">
                                <h4  class="mb-0 header-new-text" style="line-height:25px">Edit Asset Type
</h4>
                                <p class="mb-0  header-new-subtext" style="line-height:15px"><?php echo date('Y-M-d').' at '.date('H:i:s').' GMT'  ?> by {{Auth::user()->firstname.' '.Auth::user()->lastname}}</p>
                                    ...
                                    <div class="new-header-icon-div">
                                             ...
                                             <a  href="javascript:;"   data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Comment"><img src="{{asset('public/img/comment-white.png')}}" width="20px"></a>
                                              ...
                                              <a href="javascript:;" class="text-white saveContract" data="0" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Save Asset Type"><i class="fa fa-check text-white"   ></i> </a>
    ...
    padding-right: 15mm;">
                    ...
                    <form  id="form-1" action="{{url('insert-client')}}" class="js-validation   " method="POST" enctype="multipart/form-data"  >
                        @csrf
                  <input type="hidden" name="id" value="{{$qry->asset_type_id}}">
                        ...
                        <input type="hidden" name="attachment_array" id="attachment_array" >
                                <div class="row">
                                  ...
                                  <div class="form-group row">
                                        ...
                                        <input type="name" class="form-control" id="asset_type_description" name="asset_type_description"  value="{{$qry->asset_type_description}}" >
                                            </div>
     ...
     <div class="form-group row">
        ...
        <div class="avatar-edit">
            <input type='file' id="imageUpload" name="asset_icon" accept=".png, .jpg, .jpeg" />
            <label for="imageUpload"></label>
        </div>
        <div class="avatar-preview">
            <div id="imagePreview" style="background-image: url('{{asset('public/asset_icon/')}}/{{$qry->asset_icon}}');">
            </div>
            <input type="hidden" name="hidden_img" value="{{$qry->asset_icon}}">
        </div>
    </div>
</div>
 </div></div></div>
...
var asset_type_description=$('option:selected',$('select[name=cert_hostname]')).attr('data');
var asset_name=$('option:selected',$('select[name=cert_hostname]')).text();
asset_icon=asset_icon==null?'':asset_icon;
asset_type_description=asset_type_description==null?'':asset_type_description;
...
Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/warning-yellow.png')}}" width="30px" class="mt-n1"> You must select a hostname.', delay: 5000});
        ...
        Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/warning-yellow.png')}}" width="30px" class="mt-n1"> You must select at least one IP.', delay: 5000});
           ...
           hostArray.push({key:host_key_count,hostname:host,asset_icon:asset_icon,asset_type_description:asset_type_description,asset_name:asset_name,ip:ip});
...
var asset_type_description=$('option:selected',$('select[name=cert_hostname_edit')).attr('data');
...
Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/warning-yellow.png')}}" width="30px" class="mt-n1"> You must select a hostname.', delay: 5000});
        ...
        Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/warning-yellow.png')}}" width="30px" class="mt-n1"> You must select at least one IP.', delay: 5000});
           ...
           hostArray[id].asset_type_description=asset_type_description;
                                        ...
                                        <table class="table table-borderless table-vcenter mb-0">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="js-task-content  pl-0">
                                                        <h2 class="mb-0 comments-text">${hostArray[i].asset_name}<br><span class="comments-subtext">${hostArray[i].asset_type_description}
</span></h2>
                                                    </td>
...
if(click==1){
   Dashmix.helpers('notify', {message: 'Close window?  <a href="javascript:;" class="  btn-notify btnCloseUndo ml-4" >Proceed</a>', delay: 5000});
        ...
        window.location.href=unEntity('{{URL::previous()==URL::full()?url('asset_types'):URL::previous()}}');
...
var data1=$(this).attr('data');
        var asset_type_description=$('#asset_type_description').val();
     

 if(asset_type_description==''){
...
if(data1==0){
        ...
        Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Asset Type Succesfully added', delay: 5000});

       
     window.location.href=unEntity('{{URL::previous()==URL::full()?url('asset_types'):URL::previous()}}');
}else{
      Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Asset Type Succesfully added.<br><span  style="margin-left:32px;color:lightgrey">Continue adding Asset Type</span>', delay: 5000});
}
...
$.ajax({
    ...
    url:"{{url('get-comments-asset-type')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
...
$.ajax({
    ...
    'method':'get',
    data:{id:'<?php echo $_GET['id'] ?>'},
    url:"{{url('get-attachment-asset-type')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
            var newDate=new Date(date);
