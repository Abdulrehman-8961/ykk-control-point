...
<?php $qry= DB::Table('domains')->where('id',$_GET['id'])->first();?>
                ...
                <!-- Hero -->
                            ...
                            <div class="block card-round   bg-new-blue new-nav" >
                               ...
                               <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                <div class="d-flex">
                                <img src="{{asset('public/img/header-white-domains.png')}}" width="40px">
                                <div class="ml-4">
                                <h4  class="mb-0 header-new-text" style="line-height:25px">Edit Domain
</h4>
                                <p class="mb-0  header-new-subtext" style="line-height:15px"><?php echo date('Y-M-d').' at '.date('H:i:s').' GMT'  ?> by {{Auth::user()->firstname.' '.Auth::user()->lastname}}</p>
                                    ...
                                    <div class="new-header-icon-div">
                                             ...
                                             <a  href="javascript:;"   data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Comment"><img src="{{asset('public/img/comment-white.png')}}" width="20px"></a>
                                              ...
                                              <a href="javascript:;" class="text-white saveContract" data="0" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Save Domain"><i class="fa fa-check text-white"   ></i> </a>
                        ...
                        <input type="hidden" name="attachment_array" id="attachment_array" >
                                <div class="row">
                                  ...
                                  <div class="form-group row">
                                        ...
                                        <?php
                                            $client=DB::Table('clients')->where('is_deleted',0)->where('client_status',1)->get();
                                             ?>
 <select type="client_id" class="form-control select2" id="client_id"  value="" name="client_id" placeholder="Client"  >
                                                    <option value=""></option>
                                                    @foreach($client as $c)
                                                    <option value="{{$c->id}}"  {{$c->id==$qry->client_id?'selected':''}}>{{$c->firstname}}</option>
                                                    @endforeach
                                                    </select>
                                            </div>

                                          </div>
<input type="hidden" name="domain" value="{{$qry->id}}">

                                  <div class="form-group row">
                                            <label class="col-sm-2  col-form-label mandatory" for="example-hf-client_id">Name</label>
                                          
                                            <div class="col-sm-6  form-group">
                                      
   <input type="text" class="form-control" id="domain_name" name="domain_name" value="{{$qry->domain_name}}" placeholder="Domain Name"  >
                                            </div>
...
$.ajax({
    ...
    'method':'get',
    data:{id:'<?php echo $_GET['id'] ?>'},
    url:"{{url('get-attachment-domains')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
            var newDate=new Date(date);
...
if(click==1){
   Dashmix.helpers('notify', {message: 'Close window?  <a href="javascript:;" class="  btn-notify btnCloseUndo ml-4" >Proceed</a>', delay: 5000});
        ...
        window.location.href=unEntity('{{URL::previous()==URL::full()?url('domains'):URL::previous()}}');
...
if(data1==0){
        ...
        Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Domain Succesfully updated', delay: 5000});

       
    // window.location.href=unEntity('{{URL::previous()==URL::full()?url('domains'):URL::previous()}}');
}else{
      Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Domain Succesfully added.<br><span  style="margin-left:32px;color:lightgrey">Continue adding Domain</span>', delay: 5000});
}
