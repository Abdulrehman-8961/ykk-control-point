...
?>
<?php 
$qry=DB::Table('operating_systems')->where('id',$_GET['id'])->first();
?>
                ...
                <!-- Hero -->
                            ...
                            <div class="block card-round   bg-new-blue new-nav" >
                               ...
                               <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                <div class="d-flex">
                                <img src="{{asset('public/img/header-white-os.png')}}" width="40px">
                                <div class="ml-4">
                                <h4  class="mb-0 header-new-text" style="line-height:25px">New Operating System
</h4>
                                <p class="mb-0  header-new-subtext" style="line-height:15px"><?php echo date('Y-M-d').' at '.date('H:i:s').' GMT'  ?> by {{Auth::user()->firstname.' '.Auth::user()->lastname}}</p>
                                    ...
                                    <div class="new-header-icon-div">
                                             ...
                                             <a  href="javascript:;"   data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Comment"><img src="{{asset('public/img/comment-white.png')}}" width="20px"></a>
                                              ...
                                              <a href="javascript:;" class="text-white saveContract" data="0" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Save Operating System"><i class="fa fa-check text-white"   ></i> </a>
    ...
    padding-right: 15mm;">
                        ...
                        <input type="hidden" name="attachment_array" id="attachment_array" >
                                <div class="row">
                                  ...
                                  <div class="form-group row">
                                        ...
                                        <input type="name" class="form-control" id="operating_system_name" name="operating_system_name"  value="{{$qry->operating_system_name}}" >
                                            </div>
     ...
     <div class="form-group row">
        ...
        <div class="avatar-edit">
            <input type='file' id="imageUpload" name="operating_system_image" accept=".png, .jpg, .jpeg" />
            <label for="imageUpload"></label>
        </div>
        <div class="avatar-preview">
            <div id="imagePreview" style="background-image: url('{{asset('public/operating_system_logos/')}}/{{$qry->operating_system_image}}');">
            </div>
            <input type="hidden" name="hidden_img" value="{{$qry->operating_system_image}}">
        </div>
    </div>
</div>
 </div></div></div>
...
if(click==1){
   Dashmix.helpers('notify', {message: 'Close window?  <a href="javascript:;" class="  btn-notify btnCloseUndo ml-4" >Proceed</a>', delay: 5000});
        ...
        window.location.href=unEntity('{{URL::previous()==URL::full()?url('operating_systems'):URL::previous()}}');
...
var data1=$(this).attr('data');
        var operating_system_name=$('#operating_system_name').val();
...
$.ajax({
    type:'post',
    data:formData,
    'url':'{{url('update-operating-systems')}}',
    dataType:'json',
    async:false,
...
if(data1==0){
        ...
        Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Operating System Succesfully added', delay: 5000});

       
   //  window.location.href=unEntity('{{URL::previous()==URL::full()?url('operating_systems'):URL::previous()}}');
}else{
      Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> Operating System Succesfully added.<br><span  style="margin-left:32px;color:lightgrey">Continue adding Operating System</span>', delay: 5000});
}
...
$.ajax({
    ...
    url:"{{url('get-comments-operating-systems')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
...
$.ajax({
    ...
    'method':'get',
    data:{id:'<?php echo $_GET['id'] ?>'},
    url:"{{url('get-attachment-operating-systems')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
            var newDate=new Date(date);
