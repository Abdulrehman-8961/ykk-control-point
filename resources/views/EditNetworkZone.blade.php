...
if(Auth::user()->role=='read'){
 ...
 $qry=DB::table('network_zone')->where('id',$_GET['id'])->first();
                ...
                <!-- Hero -->
                            ...
                            <div class="block card-round   bg-new-blue new-nav" >
                               ...
                               <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                <div class="d-flex">
                                <img src="{{asset('public/img/header-network-segment-white.png')}}" width="40px">
                                <div class="ml-4">
                                <h4  class="mb-0 header-new-text" style="line-height:25px">Edit Network Zone</h4>
                                    ...
                                    <div class="new-header-icon-div">
                                         <span data-toggle="modal" data-bs-target="#AttachmentModal" data-target="#AttachmentModal" > 
                                             ...
                                             <a  href="javascript:;"   data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Comment"><img src="{{asset('public/img/comment-white.png')}}" width="20px"></a>
                                              ...
                                              <a href="javascript:;" class="text-white saveContract"  data="0"  data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Save Network Zone"><i class="fa fa-check text-white"   ></i> </a>
   ...
   <div class="block new-block" >
                            ...
                            <div class="block-content new-block-content">
                                      ...
                                      <div class="row form-group ">
                                            ...
                                            <div class="col-sm-4">
                                                      <input type="text" class="form-control"  id="network_zone_description"   name="network_zone_description"  value="{{$qry->network_zone_description}}" placeholder=""  > 
                                            </div>
                                                ...
                                                <div class="row form-group ">
                                                         <label class="col-sm-2  mandatory col-form-label" for="example-hf-email">Tag Back Color </label>
                                            <div class="col-sm-2">
                                                  <input type="color" class="form-control"  id="tag_back_color"   name="tag_back_color" placeholder=""   value="{{$qry->tag_back_color}}"> 
                                            </div>
                                                ...
                                                <div class="row form-group ">
                                                         <label class="col-sm-2  mandatory col-form-label" for="example-hf-email">Tag Text Color  </label>
                                            <div class="col-sm-2">
                                                  <input type="color" class="form-control"  id="tag_text_color" value="{{$qry->tag_text_color}}" name="tag_text_color" placeholder=""  > 
                                            </div>
                                                    
                                                          <div class="col-sm-2">
                                                 <div style="box-shadow:none;padding-bottom: 0px;padding-top:0px;min-height:29px;width:!important;border-radius:5px;background:{{$qry->tag_back_color}};color:{{$qry->tag_text_color}}" class=" text-center px-2 border-none  font-size-md badgeAuto  bubble-white-new bubble-text-sec">{{$qry->network_zone_description}}</div>
                                                 <div>
                                              <p class="text-secondary text-center">Preview</p></div>
                                            </div> 
                                                    
                                        </div>
                                         <div class="row form-group ">
                                                         <label class="col-sm-2  mandatory col-form-label" for="example-hf-email">  </label>
...
if(click==1){
   Dashmix.helpers('notify', {message: 'Close window?  <a href="javascript:;" class="  btn-notify btnCloseUndo ml-4" >Proceed</a>', delay: 5000});
    ...
    window.location.href='{{url('network-zone')}}';
}
})
$(document).on('click','.btnCloseUndo',function(){
        window.location.href='{{URL::previous()==URL::full()?url('network-zone'):URL::previous()}}';
})
...
$('input').on('keyup',function(){
    var bg=$('input[name=tag_back_color]').val()
    var color=$('input[name=tag_text_color]').val()
    var network_zone_description=$('input[name=network_zone_description]').val()
        $('.badgeAuto').addClass('contract_type_button')
        $('.badgeAuto').css({'color':color,'backgroundColor':bg})
        $('.badgeAuto').html(network_zone_description)
})
...
var data1=$(this).attr('data');
        var network_zone_description=$('#network_zone_description').val();
        
 
     
 if(network_zone_description==''){
Dashmix.helpers('notify', {message: '<img src="{{asset('public/img/warning-yellow.png')}}" width="30px" class="mt-n1"> Please enter a value of Description.', delay: 5000});
...
if(data1==0){
        ...
        Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> network zone Succesfully updated', delay: 5000});

       
    // window.location.href=unEntity('{{URL::previous()==URL::full()?url('network_zone'):URL::previous()}}');
}else{
      Dashmix.helpers('notify', {align:'center',message: '<img src="{{asset('public/img/green-check.png')}}" width="30px" class="mt-n1"> network zone Succesfully added.<br><span  style="margin-left:32px;color:lightgrey">Continue adding network zone Type</span>', delay: 5000});
}
    ...
    $.ajax({
                ...
                var html='';
                    for(var i=0;i<res.length;i++){
                        ...
                        if(i==0){
                            $('#renewal_notification_email').val(res[i].renewal_email)
                        }
                        else{
                                               html+=' <div class="form-group emailRows row" data="'+l+'">'+
                                          
                                            '<div class="col-sm-12"><div class="input-group">'+
                                                  '<input type="email" class="form-control"   name="notification_renewal_email[]" value="'+res[i].renewal_email+'"   placeholder="Email '+(l+parseInt(1))+'"  >'+
...
$.ajax({
    ...
    url:"{{url('get-comments-network-zone')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
...
$.ajax({
    ...
    'method':'get',
    data:{id:'<?php echo $_GET['id'] ?>'},
    url:"{{url('get-attachment-network-zone')}}",
    success:function(res){
        for(var i=0;i<res.length;i++){
            var date=res[i].date;
            var newDate=new Date(date);
