                        ...
                        <div class="row">
                            <div class="col-8 search-col row" style="position: relative;">
                                <div class="d-flex align-items-center" style="padding-left: 20px;">
                                    <img src="{{ asset('public/img/gl-menu-icons/enquiry-by-source-white.png') }}"
                                ...
                                <div class="float-left search-col row d-flex flex-nowrap" style="width: 70%;">
                                    <div class="col-sm-3 d-flex align-items-center justify-content-end"
                                        ...
                                        <div class="source-container position-relative">
                                            <span id="source-text">CC-B</span>
                                            <a id="btn-prev-source" class="position-absolute btn">
                                                <img src="{{ asset('public') }}/icons/left_arrow_clean.png" width="25"
                                                    alt="">
                                            </a>
                                            <a id="btn-next-source" class="position-absolute btn">
                                    ...
                                    <input type="hidden" name="source_id" id="source_id" value="15">
    ...
    <script type="text/javascript">
        ...
        `;
            ...
            $(document).on('click', '.btn-export-confirm-current', function() {
                ...
                var url = '{{ url('journal-by-source-export') }}';
                if (client_id && client_fyear && period && source_id && source_text) {
                    var $btn = $(this);
            ...
            $(document).on('click', '.btn-export-confirm', function() {
                ...
                var source_text = $('#source_text').val();
                var url = '{{ url('journal-by-source-export') }}';
                if (client_id && client_fyear && source_id && source_text) {
                    var $btn = $(this);
                    var originalContent = $btn.html();
                    if ($btn.hasClass('disabled')) {
                        return;
            ...
            function loadSources() {
                ...
                $.ajax({
                    url: '{{ url('/get-sources') }}', // Laravel route
                    type: 'POST',
                    data: {
                        client: client,
                        year: year,
                    ...
                    success: function(response) {
                        let sourceData = response.sources;
                        let currentIndex = sourceData.length > 0 ? 0 : -1;

                        function updateSource(index) {
                            if (index >= 0 && index < sourceData.length) {
                                $("#source-text").text(sourceData[index].source_code);
                                $("#source_id").val(sourceData[index].id);
                                $("#source_text").val(sourceData[index].source_code);
                                currentIndex = index;
                            }
                            getContent();
                        }
                        // Previous Button Click
                        $("#btn-prev-source").off('click').on('click', function() {
                            if (sourceData.length === 0) return;
                            let prevIndex = (currentIndex - 1 + sourceData.length) % sourceData
                                .length;
                            updateSource(prevIndex);
                            $("#select_all").prop('checked', false);
                            $("#btn_batch_update").prop('disabled', true);
                            $("#btn_batch_delete").prop('disabled', true);
                        });
                        // Next Button Click
                        $("#btn-next-source").off('click').on('click', function() {
                            if (sourceData.length === 0) return;
                            let nextIndex = (currentIndex + 1) % sourceData.length;
                            updateSource(nextIndex);
                            $("#select_all").prop('checked', false);
                            $("#btn_batch_update").prop('disabled', true);
                            $("#btn_batch_delete").prop('disabled', true);
                        });
