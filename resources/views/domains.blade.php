...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['domains'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'domains'=>$limit]);
        }
...
$field=$_GET['field'];
     ...
     $qry=DB::table('domains as s')->where('s.is_deleted',0)->select('s.*','c.firstname','c.logo')->join('clients as c','c.id','=','s.client_id')->where(function($query){
...
$qry=DB::table('domains as s')->select('s.*','c.firstname','c.logo')->where('s.is_deleted',0)->join('clients as c','c.id','=','s.client_id') ->orderBy('s.id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('domains/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Domains">
...
<div class="col-sm-3  1"  style="margin-top: 2px;" >
    <span data-toggle="modal" data-bs-target="#filterModal" data-target="#filterModal"> 
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-domains')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Domains">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('domains')}}?{{$_SERVER['QUERY_STRING']}}">
         ...
         <img src="{{asset('/public')}}/img/image-default.png" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                            ...
                            <p class="font-12pt mb-0 text-truncate c1"><b> {{$q->firstname}}
                                                      ...
                                                      <div class="d-flex">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >D</p>
                                                             
                                                                         <p class="font-12pt mb-0 text-truncate   c4"   style="min-width: 100%;max-width: 100% " data="{{$q->id}}">{{$q->domain_name}} </p>
                                                                       </div>

                              

                                          <p class="font-12pt mb-0 text-truncate  c2">Domain</p>
     ...
     @else
          ...
          <?php   
                                    ...
                                    if($q->domain_status!='1'){
                                                   ...
                                                   ?>
   <a class="dropdown-item d-flex align-items-center px-0 btnEnd  "     data="{{$q->id}}" href="javascript:;"  data-id="{{$q->id}}" data-ended="1">     <div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/d3-reactivate.png?cache=1"  width="22px" ></div> Reactivate</a>

                                                 <?php }else {  ?>
                                                  <a class="dropdown-item d-flex align-items-center px-0 btnEnd  "     data="{{$q->id}}" href="javascript:;"  data-id="{{$q->id}}">     <div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/revoke-removebg-preview.png?cache=1"  width="22px" ></div> Deactive</a>

                                                 <?php } 
       ?>
                    
                  <a class="dropdown-item d-flex align-items-center  px-0"    href="{{url('edit-domains')}}?id={{$q->id}}" ><div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/d3-edit.png?cache=1" width="17px"  ></div> Edit</a>
        ...
        <form class="mb-0 pb-0"  action="{{url('end-domains')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header endTitle">Revoke Domains</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Domains</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
$(document).on('click','.btnEnd',function(){
    ...
    $('.endTitle').html('Reactive Domain')
    }
    else{
      $('.endTitle').html('Deactive Domain')
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Domains");
                    if(c){
                        window.location.href="{{url('delete-domains')}}?id="+id;
                    }
                            })  
           })
</script>
