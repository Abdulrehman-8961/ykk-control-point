...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['distributors'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'distributors'=>$limit]);
        }
        ...
        if($no_check!=''){
            if($no_check->distributors!=''){
            $limit=$no_check->distributors;
...
$qry=DB::table('distributors') ->where('is_deleted',0)->orderBy('id','desc')->paginate($limit); 
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('distributors/')}}?{{$filter}}">
                                        
                                <div class="input-group">
                                    <input type="text" value="{{@$_GET['search']}}" class="form-control searchNew" name="search" placeholder="Search Distributor">
...
<div class="col-sm-3  1"   style="margin-top: 2px;">
                         ...
                         <a class="btn btn-dual  d2 "    href="{{url('add-distributors')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add Distributor">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('distributors')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
                         ...
                         <div class="block-content pt-1 pb-1 d-flex  pl-1 position-relative">
                                            ...
                                            @if($q->distributor_image!='')
                                                    <img src="{{asset('/public')}}/distributor_logos/{{$q->distributor_image}}" class="rounded-circle"  width="100%" height="60px" style=" object-fit: cover;">
                                                     @else
                                        ...
                                        <div class="  " style="width:55%">
                                                      ...
                                                      <div class="d-flex " style="padding-top: 20px">
                                                                    ...
                                                                    <p class="font-11pt mr-1   mb-0  c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip"  >D</p>
                                                                         ...
                                                                         <p class="font-12pt mb-0 text-truncate   c4"    data="{{$q->id}}"  style="max-width:100%;min-width: 100%">{{$q->distributor_name}} </p>
                                                                       </div>
        ...
        <form class="mb-0 pb-0"  action="{{url('end-distributors')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Revoke Distributor</span>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  Distributor</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
function showData(id){
    ...
    $('.c4').css({'backgroundColor':'#D9D9D9','color':'#7F7F7F','borderColor':'#7F7F7F'})
        ...
        $.ajax({
            type:'get',
            data:{id:id},
            url:'{{url('get-distributor-content')}}',
                dataType:'json',
    beforeSend() {
...
var distributor_id='<?php echo isset($_GET['cert_issuer'])?implode(',',$_GET['cert_issuer']):''?>';
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this Distributor");
                    if(c){
                        window.location.href="{{url('delete-distributors')}}?id="+id;
                    }
                            })  
           })
</script>
