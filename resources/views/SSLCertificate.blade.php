...
if(isset($_GET['limit']) && $_GET['limit']!=''){
    $limit=$_GET['limit'];
        ...
        if($no_check!=''){
            DB::table('settings')->where('user_id',Auth::id())->update(['ssl_certificate'=>$limit]);
        }
        else{
            DB::table('settings')->insert(['user_id'=>Auth::id(),'ssl_certificate'=>$limit]);
        }
        ...
        if($no_check!=''){
            if($no_check->ssl_certificate!=''){
            $limit=$no_check->ssl_certificate;
...
$field=$_GET['field'];
 ...
 if(isset($_GET['cert_status'])  && $_GET['cert_status']!=''){
                ...
                else if($cert_status=='Expired/Ended'){
                  $cond.=" and (s.cert_status='Ended' || s.cert_status='Expired/Ended')";
                }
                else{
...
if(Auth::user()->role=='admin'){
     ...
     $qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.firstname','i.vendor_name as issuer','i.vendor_image','c.logo')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->where(function($query) use ($sear){
...
$query->Orwhere('cert_company','like','%'.@$_GET['search'].'%');
   ...
   $query->Orwhere('cert_state','like','%'.@$_GET['search'].'%');
    $query->Orwhere('cert_email','like','%'.@$_GET['search'].'%'); 
     ...
     })->whereRaw("s.is_deleted=0 $cond")->orderBy($field,$orderby)->paginate($limit); 
 }
 else{
        $qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.firstname','i.vendor_name as issuer','i.vendor_image','c.logo')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->where(function($query) use ($sear){
             $query->Orwhere('description','like','%'.@$_GET['search'].'%');
...
$query->Orwhere('cert_company','like','%'.@$_GET['search'].'%');
   ...
   $query->Orwhere('cert_state','like','%'.@$_GET['search'].'%');
    $query->Orwhere('cert_email','like','%'.@$_GET['search'].'%'); 
     ...
     })->whereRaw("s.is_deleted=0 $cond")->whereIn('s.client_id',$userAccess)->orderBy($field,$orderby)->paginate($limit);
    ...
    if(Auth::user()->role=='admin'){
$qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.firstname','i.vendor_name as issuer','i.vendor_image','c.logo')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->where('s.is_deleted',0)->orderBy('s.id','desc')->paginate($limit); 
}
else{
...
?>
               ...
               <form class="push mb-0"   method="get" id="form-search" action="{{url('ssl-certificate/')}}?{{$filter}}">
...
<div class="col-sm-3  1"   style="margin-top: 2px;"  >
    <span data-toggle="modal" data-bs-target="#filterModal" data-target="#filterModal"> 
      <button type="button" class="btn btn-dual d1 {{isset($_GET['advance_search'])?'active':''}} "   data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Filters"   >
                           ...
                           <button class="btn btn-dual d2     " style="margin-left: 5px;margin-right:  0px"    data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Export"   href="javascript:;" style="margin-left: 5px;margin-right:  0px" >
                           <img src="{{asset('public/img/ui-icon-export.png')}}" width="20px" height="20px">
                        </button>
                    </span>
                              @if(Auth::user()->role!='read') 
                         <a class="btn btn-dual   d2 "    href="{{url('add-ssl-certificate')}}" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Add SSL Certificate">
...
<div class="col-sm-3 ">
 ...
 <div class="d-flex text-right col-lg-3 justify-content-end" ><form  id="limit_form" class="ml-2 mb-0"  action="{{url('ssl-certificate')}}?{{$_SERVER['QUERY_STRING']}}">
...
<div class="block block-rounded   table-block-new mb-2 pb-0 -  viewContent" data="{{$q->id}}" style="cursor:pointer;">
         ...
         <img src="{{asset('/public')}}/vendor_logos/{{$q->vendor_image}}" class="rounded-circle"  width="100%" style=" object-fit: cover;">
                            ...
                            <p class="font-12pt mb-0 text-truncate c1"><b> {{$q->firstname}}
                                                         </b></p>
                                             
                                                      <div class="d-flex">
                                                                    @if($q->cert_type=='internal')
                                                                    <p class="font-11pt mr-1   mb-0  c4-p  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip" data-title="Internal Certificate">I</p>
                                                                    
                                                                  @else

                                                                     <p class="font-11pt mr-1   mb-0   c4-v  "  style="max-width:12%; " data="{{$q->id}}" data-toggle="tooltip" data-title="Public Certificate">P</p>
                                                                    @endif
                                                                         <p class="font-12pt mb-0 text-truncate   c4"  style="max-width:100%;min-width: 100%" data="{{$q->id}}">{{$q->cert_name}}</p>
                                                                       </div>

                              

                                          <p class="font-12pt mb-0 text-truncate   c2" style="width: 90%">{{$q->description}}</p>
                                        ...
                                        <div class=" text-right" style="width:25%;;">
                                                                       ...
                                                                       <?php $cert_edate=date('Y-M-d',strtotime($q->cert_edate)); 
             $today=date('Y-m-d');
             $earlier = new DateTime($q->cert_edate);
...
$abs_diff = $later->diff($earlier)->format("%a"); //3
 ...
 -->
                    ...
                    <?php $line_items=DB::Table('ssl_host as ca')->select('a.hostname','a.AssetStatus')->where('ca.ssl_id',$q->id)->join('assets as a','a.id','=','ca.hostname')->groupBy('ca.hostname') ->orderBy('a.hostname','asc')->get();
                                                                            ...
                                                                            foreach($line_items as $l){
                                                                                if($l->AssetStatus!='1'){
                                                                                            ...
                                                                                            $san_items=DB::Table('ssl_san')->select( 'san')->where('ssl_id',$q->id)  ->groupBy('san') ->orderBy('san','asc')->get();
                                                                            $cvm1='<b class="HostActive text-white ">SANs</b><br>';
                                                                            foreach($san_items as $l){
                                                                                 
                                                                                                        $cvm1.='<span class="SSLActive">'.$l->san.'</span><br>';    
                                                                                }
                                                                 ?>
                                                                      <div  class="" style="position: absolute;width: 100%; bottom:2px;right: 10px;display: flex;align-items: center;justify-content: end;">
                                                                            
                                                                      <div  class="ActionIcon" style="margin-left:2px;" data-src="{{asset('public')}}/img/icon-san-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-san-grey-darker.png?cache=1">
                                                                
                                                                       <a href="javascript:;" class="toggle" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="{{$cvm1}}" data-html="true"   data-original-title="{{$cvm1}}" >
                                                                        <img  src="{{asset('public')}}/img/icon-san-grey-darker.png?cache=1"  height="24px">
                                                                        </a>
                                                                    </div>
 ...
 <a href="javascript:;" class="toggle " data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="<p class='HostActive text-white  my-0'>Validity Range</p><p class='HostActive my-n1 text-orange  '>{{date('d-M-Y',strtotime($q->cert_rdate))}}-{{date('d-M-Y',strtotime($q->cert_edate))}}</p><p class='font-10pt mb-0 text-grey text-truncate mt-0 '> <small><i>{{$abs_diff}} days remaining</i></small></p>" data-html="true" data-original-title="" >
                             <img  src="{{asset('public')}}/img/calendar-grey-removebg-preview.png?cache=1" width="24px"  class="   " >
                                                                    ...
                                                                    <div class="ActionIcon"  data-src="{{asset('public')}}/img/icon-hosts-grey-darker.png?cache=1" data-original-src="{{asset('public')}}/img/icon-hosts-grey-darker.png?cache=1">
                                                                        ...
                                                                        <img  src="{{asset('public')}}/img/icon-hosts-grey-darker.png?cache=1" width="24px"   >
                                                                        </a>
                                                                    </div>
                                                                                          <?php     if(Auth::check()){    
                                            if(@Auth::user()->role!='read'){ ?>
          ...
          <?php   
                                    ...
                                    if($q->cert_status!='Inactive' && $q->cert_status!='Expired/Ended' && $q->cert_status!='Ended'){
                                                   ...
                                                   ?>
   <a class="dropdown-item d-flex align-items-center px-0 btnEnd  "     data="{{$q->id}}" href="javascript:;"  data-id="{{$q->id}}">     <div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/revoke-removebg-preview.png?cache=1"  width="22px" ></div> Revoke Certificate</a>

                                                 <?php }else if($q->cert_status=='Expired/Ended' || $q->cert_status=='Ended'){  ?>
                                                  <a class="dropdown-item d-flex align-items-center px-0 btnEnd  "     data="{{$q->id}}" href="javascript:;"  data-id="{{$q->id}}" data-ended=1>     <div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/d3-reactivate.png?cache=1"  width="22px" ></div> Reinstate Certificate</a>

                                                 <?php } 
    
                                                        if($q->cert_status!='Inactive' && $q->cert_status!='Expired/Ended' && $q->cert_status!='Ended'){?>
                  <a class="dropdown-item d-flex align-items-center px-0  "  data="" href="{{url('renew-ssl-certificate')}}?id={{$q->id}}"  data-id="{{$q->id}}">     <div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/refresh_icon-removebg-preview.png?cache=1"  width="22px" ></div> Renew Certificate</a>
                         <?php }

                                                 ?>
                  <a class="dropdown-item d-flex align-items-center px-0" href="print-ssl-certificate?id={{$q->id}}">   <div style="width: 32;  padding-left: 2px"><img src="{{asset('public')}}/img/dr-pdf.png?cache=1" width="26px"  > PDF</div></a>  
                  ...
                  <a class="dropdown-item d-flex align-items-center  px-0"    href="{{url('edit-ssl-certificate')}}?id={{$q->id}}" ><div style="width: 32px;  padding-left: 5px"><img src="{{asset('public')}}/img/d3-edit.png?cache=1" width="17px"  ></div> Edit</a>
        ...
        <form class="mb-0 pb-0"  action="{{url('end-ssl-certificate')}}" method="post" >
            @csrf
                ...
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header endTitle">Revoke SSL Certificate</span>
    ...
    <div class="col-sm-12">
          <textarea class="form-control" rows="5" required="" name="reason" id="reason"></textarea>
                        ...
                        <div class="block-content block-content-full   pt-4" style="padding-left: 9mm;padding-right: 9mm">
                            <button type="submit" class="btn mr-3 btn-new"     >OK</button>
...
<div class="modal fade  " id="filterModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog  modal-dialog-fadeIn modal-dialog-centered modal-lg modal-bac  " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Filter SSL Cetificate</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
   ...
   <div class="row">
                                            ...
                                            <label class="   " for="example-hf-email">Cert Status</label>
                                         
                                        <select type="text" class="form-control" value="{{@$_GET['cert_status']}}" id="cert_status" name="cert_status" placeholder="All "  > 
                                                            <option value="">All</option>
                                                    
                                                            <option value="Active" {{@$_GET['cert_status']=='Active'?'selected':''}}>Active</option>\
                                                            <option value="Upcoming" {{@$_GET['cert_status']=='Upcoming'?'selected':''}}>Upcoming</option>
                                                            <option value="Inactive" {{@$_GET['cert_status']=='Inactive'?'selected':''}}>Renewed</option>
                                                            <option value="Expired" {{@$_GET['cert_status']=='Expired'?'selected':''}}>Expired</option>
                                                            <option value="Expired/Ended" {{@$_GET['cert_status']=='Expired/Ended'?'selected':''}}>Revoked</option>
    ...
    <div class="col-sm-4  form-group">
     ...
     <div class="col-sm-4 form-group">
                                            <label class="   " for="example-hf-email">  Type</label>
                                         
                                                 <select type="text" class="form-control" value="{{@$_GET['cert_type']}}" id="cert_type" name="cert_type" placeholder="All "  > 
                                                            <option value="">All</option>
                                                <option value="internal" {{@$_GET['cert_type']=='internal'?'selected':''}}>Internal</option>
                                                            <option value="public" {{@$_GET['cert_type']=='public'?'selected':''}}>Public</option>
                           ...
                           <div class="col-sm-4  form-group">
                                            <label class="   " for="example-hf-client_id">Issuer</label>
                                          
                                          
                                                 <select type="" class="form-control selectpicker " id="cert_issuer"  data-style="btn-outline-light border text-dark"  data-actions-box="true"  data-live-search="true"  title="All" value="" name="cert_issuer[]" multiple=""   >
                                                     <?php
                                            $vendor=DB::Table('vendors')->where('is_deleted',0) ->orderBy('vendor_name','asc')->get();

                                                          $vendorArray=$_GET['cert_issuer'] ?? [];
                                             ?>
                                                         @foreach($vendor as $s)
 ...
 <a href="{{url('ssl-certificate')}}" class="btn     btn-new-secondary float-right" style="background: black;
    color: goldenrod;">Clear Filters</a>
                                            @else
                                                
     <a href="{{url('certificate')}}" class="btn     btn-new-secondary float-right" style="">Clear Filters</a>
                                       @endif
                        </div>
                    </div>
...
<div class="modal fade" id="ExportModal"  tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="modal-block-large" aria-hidden="true">
            <div class="modal-dialog modal- -centered  modal-md modal-bac  " role="document">
                <div class="modal-content">
                    <div class="block  block-transparent mb-0">
                        <div class="block-header pb-0  ">
                            <span class="b e section-header">Export  SSL Certificate</span>
                            <div class="block-options">
                              <!--   <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                </button> -->
...
var vendor_id='<?php echo isset($_GET['cert_issuer'])?implode(',',$_GET['cert_issuer']):''?>';
...
$(document).on('click','.btnEnd',function(){
    ...
    $('.endTitle').html('Reinstate SSL Certificate')
    }
    else{
      $('.endTitle').html('End SSL Certificate')
...
var vendor='';
if(res.vendor_image=='' || res.vendor_image==null){
vendor='{{asset('public/img')}}/internal-ssl.png';
}   
else{                                             
vendor='{{asset('public/vendor_logos/')}}/'+res.vendor_image;
     }
                     

                       $('#hostnameDisplay').html('<div style="display:flex;align-items:center"><img class="  mr-2 atar48" style="object-fit: cover"  src="'+vendor+'"  alt="" width="60px" height="40px"> <div><p class="text-uppercase mb-0 mt-2" style="color:#0D0D0D!important;font-size:15pt;line-height:20px"><img class="  mr-3 atar48" width="30px"    src="{{asset('public/img/')}}/'+status+'" alt=""><b>'+res.cert_name+'</b></p><p class="my-0"><span style="color:#D3D3D3!important;font-size:10pt">'+(res.cert_type!=null?res.cert_type.toUpperCase():'')+'</span></p></div></div>')
  ...
  if(res.attachment=='' || res.attachment==null){
          ...
          $(document).on('click','.btnDelete',function(){
                    ...
                    var c=confirm("Are you sure want to delete this SSL Certificate");
                    if(c){
                        window.location.href="{{url('delete-ssl-certificate')}}?id="+id;
                    }
                            })  
           })
</script>
