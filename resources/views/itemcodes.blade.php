...
@section('content')
    ...
    if (request()->filled('limit')) {
        ...
        if ($no_check) {
            DB::table('settings')
                ...
                ->update(['itemcodes' => $limit]);
        } else {
            DB::table('settings')->insert(['user_id' => $user_id, 'itemcodes' => $limit]);
        }
    } elseif ($no_check && !empty($no_check->itemcodes)) {
        $limit = $no_check->itemcodes;
    }
    ...
    $qry = DB::table('itemcodes as i')->leftJoin('itemcodes_colors as ic', 'ic.itemcode_id', '=', 'i.id')->where('i.is_deleted', 0);
    
    // Apply status filter - exact match for 1 or 0
    if (request()->has('filter_status') && in_array($status, ['0', '1'])) {
        $qry->where('i.status', (int) $status);
    }
    if (request()->has('filter_item_category')) {
        $qry->where('i.item_category', request()->get('filter_item_category'));
    }
    if (request()->has('filter_size')) {
        $qry->where('i.size', request()->get('filter_size'));
    }
    if (request()->has('filter_chain_code')) {
        $qry->where('i.chain_code', request()->get('filter_chain_code'));
    }
    
    // Apply color filter
    if (!empty($color)) {
        $qry->where('ic.color', 'like', '%' . $color . '%');
    }
    ...
    if (!empty($search)) {
        $qry->where(function ($query) use ($search) {
            $query
                ...
                ->orWhere('i.size', 'like', '%' . $search . '%')
                ->orWhere('i.chain_code', 'like', '%' . $search . '%')
    ...
    // // Build base query
    // $qry = DB::table('itemcodes')->where('is_deleted', 0);
    
    // // Apply status filter - exact match for 1 or 0
    // if (request()->has('filter_status') && in_array($status, ['0', '1'])) {
    //     $qry->where('status', (int)$status);  // Explicitly cast to integer
    // }
    
    // // Other filters (search, description) remain the same
    // if (!empty($search)) {
    //     $qry->where(function($query) use ($search) {
    ...
    $i_c = DB::table('itemcodes')->where('id', $GETID)->where('is_deleted', 0)->orderBy('id', 'desc')->first();
    ?>
    ...
    padding-bottom: 11px;">
                            ...
                            <div class="col-sm-4 search-col">
                                <div class="row">
                                    <div class="mb-0 col-sm-1 pl-0 text-cente r pr-0">
                                        <a class="btn btn-dual filterItemcodeModal d2 {{ (@$_GET['filter_status'] && $_GET['filter_status'] !== '') || !empty($_GET['filter_color']) ? 'filter-active' : '' }}"
                                        ...
                                        <form class="push mb-0" method="get" id="form-search"
                                            ...
                                            <div class="input-group main-search-input-group" id="search-container">
                                                <input type="text" value="{{ @$_GET['search'] }}"
                                                    ...
                                                    placeholder="Search Itemcodes" id="search-input">
                            ...
                            <div class="col-lg-8 pr-sm-4">
                                <div class="row align-items-center">
                                    <div class="col-auto mr-auto text-center" style="">
                                        <!-- <a class="btn btn-dual filterItemcodeModal d2 {{ !empty($_GET['filter_item_code']) || !empty($_GET['description']) ? 'filter-active' : '' }}" data-custom-class="header-tooltip"
                                        ...
                                        <!-- <a class="btn btn-dual AddTa xModal d2 " data-custom-class="header-tooltip"
                                                                                                            data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                                                                                            data-original-title="Add Itemcode" href="javascript:;" data-toggle="modal"
                                        ...
                                        @if (Auth::user()->role != 'read')
                                            <a class="btn btn-dual d2 btn-export text-white"
                                                ...
                                                data-url="{{ url('export-itemcodes') }}?{{ $_SERVER['QUERY_STRING'] }}">
                                            ...
                                            <a class="btn btn-dual  d2 "
                                                ...
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Import" id="ImportItemcodes">
                                    ...
                                    <form id="limit_form" class="ml-2 mb-0"
                                        action="{{ url('itemcodes') }}?{{ $_SERVER['QUERY_STRING'] }}">
                ...
                <div class="row">
                    <div class="col-md-4 bubble-header">
                        <div class="row align-items-center">
                            ...
                            $filter_chain_code = isset($_GET['filter_chain_code']) && trim($_GET['filter_chain_code']) !== '';
                            $filter_size = isset($_GET['filter_size']) && trim($_GET['filter_size']) !== '';
                            $filter_item_category = isset($_GET['filter_item_category']) && trim($_GET['filter_item_category']) !== '';
                            ...
                            if ($search && ($filter_status || $filter_color || $filter_chain_code || $filter_size || $filter_item_category)) {
                                ...
                                $get_text_display = 'd-block';
                            } elseif ($filter_status || $filter_color || $filter_chain_code || $filter_size || $filter_item_category) {
                            ...
                            <p class="col-11 bubble-header-text d-flex justify-content-between align-items-center">
                                ...
                                <span class="{{ $get_text_display }} text-right" style="line-height: 1.3;">
                                    <a class="clear-link" href="{{ url('/itemcodes') }}" data-custom-class="header-tooltip"
                                        data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                        ...
                        <div class="h-100 d-flex align-items-center">
                            <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                <div class="d-flex justify-content-center align-items-center">
                                    <img src="{{ asset('public/img/cf-menu-icons/main-menu-itemcodes-white.png') }}"
                                        style="width: 36px; height: 36px;">
                                    <div class="" style="margin-left: 0.91rem;">
                                        <h4 class="mb-1 header-new-text header-item-code" style="line-height:22px">Itemcode
                                        </h4>
                                ...
                                <div class="new-header-icon-div d-flex align-items-center  no-print">
                                    @if ($totalRows != 0)
                                        <a href="javascript:;" d="" class="text-white colors-img-icon"
                                            data-item-id="{{ @$_GET['id'] }}" data-item-code="" data-color-id=""
                                            data-color-value="" data-image-path="" data-toggle="tooltip"
                                            data-trigger="hover" data-placement="top" title=""
                                            data-original-title="Assign Image">
                                            <img src="{{ asset('public/img/cf-menu-icons/main-menu-image-white.png') }}"
                                                width="24px">
                                        </a>
                                        ...
                                        <!-- <a href="javascript:;" d="" class="text-white btnEdit " data="" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Edit">
                                                                                                                                                                                <img src="{{ asset('public/img/cf-menu-icons/header-edit.png') }}" width="24px">
                                                                                                                                                                            </a> -->
                                        ...
                                        <!-- <a data-toggle="tooltip" data-trigger="hover" data="" data-placement="top" title="" data-original-title="Delete" href="javascript:;" class="text-white btnDelete">
                ...
                <div class="row">
                    <div class="col-md-4 bubble-header">
                        ...
                        <div class="filter-dropdown-container roll-down">
                            <div class="row align-items-center">
                                <div class="col-1 {{ $class }}" style="height: 420px;"></div>
                                <form id="filterForm" method="GET" action="{{ url('/itemcodes') }}"
                                    ...
                                    <div class="block block-transparent mb-0">
                                        <div class="pl-3 pt-0 pb-0">
                                            <div class="align-items-baseline row">
                                                ...
                                                <label class="col-sm-4 d-flex align-items-center modal-label">Item
                                                    Category</label>
                                                <div class="col-sm-7 px-sm-0 form-group">
                                                    <select class="shadow-none form-control select2" id="filter_item_category"
                                                        name="filter_item_category">
                                                    </select>
                                                </div>
                                                {{-- /item category filter --}}
                                                {{-- item size filter --}}
                                                <label class="col-sm-4 d-flex align-items-center modal-label">Size</label>
                                                <div class="col-sm-7 px-sm-0 form-group">
                                                    <select class="shadow-none form-control select2" id="filter_size"
                                                        name="filter_size">
                                                    </select>
                                                </div>
                                                {{-- /item size filter --}}
                                                {{-- item chain code filter --}}
                                                <label class="col-sm-4 d-flex align-items-center modal-label">Chain
                                                    Code</label>
                                                <div class="col-sm-7 px-sm-0 form-group">
                                                    <select class="shadow-none form-control select2" id="filter_chain_code"
                                                        name="filter_chain_code">
                                                    </select>
                                                </div>
                                                {{-- /item chain code filter --}}
                                            </div>
                                        ...
                                        <div class="block-content text-right pt-1 pr-0" style="padding-left: 9mm;">
                                            <a href="{{ url('/itemcodes') }}" class="btn btn-action mr-3">Clear</a>
                                            ...
                                            <div class="d-flex flex-column" style="width: calc(100% - 50px)">
                                                <div>
                                                    <span class="font-signika bubble-item-title" data-toggle="tooltip"
                                                        data-trigger="hover" data-placement="top" title=""
                                                        data-original-title="Itemcode">{{ $q->item_code }}</span>
            ...
            <form class="mb-0 pb-0" action="{{ url('end-itemcode') }}" id="form-end-itemcode" method="post">
                @csrf
                        ...
                        <div class="modal-content">
                            <div class="block  block-transparent mb-0">
                                <div class="block-header pb-0  ">
                                    <span class="b e section-header"><span class="revokeText">Revoke</span> Itemcode</span>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 text-center">
                                            ...
                                            <p class="fw-300">Are you sure you wish to <span
                                                    class="revokeText text-lowercase"></span> this Itemcode?</p>
                                        </div>
                                    </div>
                                </div>
            ...
            <form class="mb-0 pb-0" action="{{ url('delete-itemcode-attachment') }}" method="post">
                @csrf
                    ...
                    <div class="modal-dialog modal-dialog-centered  -lg modal-bac " role="document">
                        <div class="modal-content">
                            <div class="block  block-transparent mb-0">
                                ...
                                <div class="block-content new-block-content pt-0 pb-0 ">
                                    <input type="hidden" id="del_itemcode_id" name="itemcode_id">
                    ...
                    <div class="modal-dialog modal-dialog-centered  -lg modal-bac " role="document">
                        <div class="modal-content">
                            <div class="block  block-transparent mb-0">
                                ...
                                <div class="block-content new-block-content pt-0 pb-0 ">
                                    <input type="hidden" id="del_comment_itemcode_id" name="itemcode_id">
            ...
            <form class="mb-0 pb-0" action="{{ url('itemcode-color-img') }}" id="form-itemcode-color-img" method="post"
                enctype="multipart/form-data">
                @csrf
                <div class="modal fade" id="update-img-modal" tabindex="-1" role="dialog" data-backd rop="static"
                    aria-labelledby="modal-block-large" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered  mod l-lg modal-bac " role="document">
                        <div class="modal-content">
                            <div class="block  block-transparent mb-0">
                                <div class="block-header   ">
                                    <span class="section-header text-uppercase modal-header-itemcode">Itemcode</span>
                                    <div class="block-options">
                                ...
                                <input type="hidden" name="modal_color_id" id="modal-color-id" value="">
                                <input type="hidden" name="itemcode_id" id="itemcode-id" value="">
                                <div class="block-content pt-0 row form-group mt-2">
                                    <label class="col-sm-4 d-flex align-items-center modal-label" for="color_img">Select
                                        Image</label>
                                    <div class="col-sm-7">
                                        <input type="file" name="color_img" id="color_img"
                                            class="modal-input form-control" accept=".jpg,.jpeg,.png,image/jpeg,image/png"
                                            required>
                                    </div>
                                </div>
            ...
            <form action="{{ url('insert-comments-itemcode') }}" method="post">
                @csrf
            ...
            <form action="{{ url('update-comments-itemcode') }}" method="post">
                @csrf
                <input type="hidden" id="edit_comment_id" name="id">
                <input type="hidden" id="edit_comment_itemcode_id" name="itemcode_id">
            ...
            <form id="itemcode-attachment-form" action="{{ url('insert-attachment-itemcode') }}" method="post">
                @csrf
            ...
            <form id="filterForm" method="GET" action="{{ url('/itemcodes') }}" class="mb-0 pb-0">
            ...
            <form action="{{ url('/import-excel-itemcodes') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal fade" id="ImportModal" tabindex="-1" role="dialog" data-b ackdrop="static"
                    ...
                    <div class="modal-dialog modal-dialog-centered mod al-lg modal-bac " role="document">
                        <div class="modal-content">
                            <div class="block  block-transparent mb-0">
                                <div class="block-header   ">
                                    <span class="b e section-header">Import Itemcodes</span>
                                    <button type="button" class="close close-cross" data-dismiss="modal"
                                        aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                ...
                                <div class="block-content row block-content-full">
                                    <div class="col-sm-7 d-flex align-items-center">
                                        <div>
                                            <a href="#" data-url="{{ asset('public/Itemcodes_sample.xlsx') }}"
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            let filePond = FilePond.create(
                document.querySelector('.attachment'), {
                    ...
                    server: {
                        process: {
                            url: '{{ url('uploadItemcodeAttachment') }}',
                            method: 'POST',
                            headers: {
                        ...
                        revert: (uniqueFileId, load, error) => {
                            ...
                            fetch(`{{ url('revertItemcodeAttachment') }}?key=${uniqueFileId}`, {
                                    method: "DELETE",
                                    body: formData,
                                })
                                .then(res => res.json())
            ...
            $('#AttachmentSave').click(function() {
                ...
                if (attachment.length === 0) {
                    ...
                    $('#itemcode-attachment-form').submit(); // Corrected selector and method
                }
            });
            ...
            $('#download-template').on('click', function(e) {
                ...
                a.href = url;
                a.download = 'Itemcodes_sample.xlsx';
            ...
            $(document).on('click', 'label.colors-radio', function() {
                const $label = $(this);
                const inputId = $label.attr('for');
                const $input = $('#' + inputId);

                $('input[name="item_code_color"]').prop('checked', false);
                $('label.colors-radio').removeClass('active');

                $input.prop('checked', true);
                $label.addClass('active');

                const newImg = $label.data('img');
                const $previewImg = $('#color-preview-img');

                if (newImg) {
                    $previewImg.attr('src', newImg).show();
                    $('.colors-img-div').show();
                } else {
                ...
                $('.colors-img-icon').attr({
                    'data-item-id': $label.data('item-id') || '',
                    'data-item-code': $label.data('item-code') || '',
                    'data-color-id': $label.data('color-id') || '',
                    'data-color-value': $input.val() || '',
                    'data-image-path': newImg || ''
                });
            });



            $(document).on('click', '.colors-img-icon', function() {
                var itemcode_id = $(this).attr('data-item-id');
                ...
                if (!color_id || color_id == "") {
                    ...
                    $('.modal-header-itemcode').text('Item' + item_code + ' ' + color);
                    $('#modal-color-id').val(color_id);
                    $('#itemcode-id').val(itemcode_id);
                }
            });

            $('#update-img-modal').on('hidden.bs.modal', function() {
                $('.colors-img-icon').removeAttr(
                    'data-item-id data-item-code data-color-id data-color-value data-image-path');
                $('#color-preview-img').attr('src', '');
                $('label.colors-radio').removeClass('active');
                $('.colors-img-div').hide();
            });
            ...
            $(document).on('click', '.delete-attachment', function() {
                ...
                $('#del_itemcode_id').val(itemcode_id);
            ...
            $(document).on('click', '.delete-comment', function() {
                ...
                $('#del_comment_itemcode_id').val(itemcode_id);
            ...
            $(document).on('click', '.edit-comment', function() {
                ...
                $('#edit_comment_itemcode_id').val(itemcode_id);
            ...
            $(document).on('click', function(e) {
                if (!$(e.target).closest('.select2-dropdown, .select2').length) {
            ...
            let selectedCategory = getUrlParameter("filter_item_category");
            let selectedChainCode = getUrlParameter("filter_chain_code");
            let selectedSize = getUrlParameter("filter_size");
            ...
            if (selectedCategory) {
                $.ajax({
                    ...
                    if (match) {
                        ...
                        $('#filter_item_category').append(option).trigger('change');
                    }
                });
            }
            if (selectedChainCode) {
                $.ajax({
                    url: "{{ url('/fetch-itemcode-chaincode') }}",
                    data: {
                        query: selectedChainCode
                    },
                    dataType: 'json'
                }).then(function(data) {
                    let match = data.find(item => item.text === selectedChainCode);
                    if (match) {
                        let option = new Option(match.text, match.text, true, true);
                        $('#filter_chain_code').append(option).trigger('change');
                    }
                });
            }
            if (selectedSize) {
                $.ajax({
                    url: "{{ url('/fetch-itemcode-size') }}",
                    data: {
                        query: selectedSize
                    },
                    dataType: 'json'
                }).then(function(data) {
                    let match = data.find(item => item.text === selectedSize);
                    if (match) {
                        let option = new Option(match.text, match.text, true, true);
                        $('#filter_size').append(option).trigger('change');
                    }
                });
            }
            ...
            setTimeout(() => {
                $('#filter_item_category').select2({
                    ...
                    templateSelection: function(data, container) {
                        ...
                        if (data.id) {
                            const optionEl = $('#filter_item_category').find("option[value='" +
                                data
                                .text + "']");
                        }
            ...
            setTimeout(() => {
                $('#filterForm #filter_size').select2({
                    ...
                    templateSelection: function(data, container) {
                        // add data attributes to the real <option>
                        if (data.id) {
                            const optionEl = $('#filterForm #filter_size').find(
                                "option[value='" +
                                data
                                .text + "']");
                        }
            ...
            setTimeout(() => {
                $('#filterForm #filter_chain_code').select2({
                    ...
                    templateSelection: function(data, container) {
                        // add data attributes to the real <option>
                        if (data.id) {
                            const optionEl = $('#filterForm #filter_chain_code').find(
                                "option[value='" +
                                data
                                .text + "']");
                        }
            ...
            function showData(id) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        $('.header-item-code').text(res.item_code);
                        ...
                        $('.colors-img-icon').attr('data-item-id', res.id);
                        ...
                        if (res.colors_img == 1) {
                            $('.colors-img-icon').show();
                        } else {
                            $('.colors-img-icon').hide();
                        }
            ...
            $(document).on('click', '.btnEnd', function() {
                ...
                $("#form-end-itemcode textarea[name=reason]").val('');
                if (status == 1) {
        ...
        $(document).on('click', '#ImportItemcodes', function() {
        ...
        $(document).ready(function() {
            $(document).on('click', '.undo-delete', function() {
                var id = $(this).attr('data');
                window.location.href = "{{ url('undo-delete-itemcode-comment') }}?id=" + id;
            });
            $(document).on('click', '.undo-delete-attachment', function() {
                var id = $(this).attr('data');
                window.location.href = "{{ url('undo-delete-itemcode-attachment') }}?id=" + id;
            });
        });
        $(document).ready(function() {
